...
STATE_MAP="$ROOT/state/filesystem-map.txt"

: > "$STATE_MAP"

if [ -d "$STATE_ROOT" ]; then

    echo "===== FILESYSTEM MAP =====" >> "$STATE_MAP"

    # Read-only bounded inventory.
    # Every filesystem race/permission error is non-fatal.
    set +e
    ...
    echo >> "$STATE_MAP"
    echo "filesystem_find_exit=$MAP_RC" >> "$STATE_MAP"

    echo >> "$STATE_MAP"
    echo "===== DIRECTORY SUMMARY =====" >> "$STATE_MAP"

    DIR_LIST="$ROOT/state/directories.tmp"
    ...
    echo >> "$STATE_MAP"
    echo "directories=$DIR_TOTAL" >> "$STATE_MAP"

else

    echo "STATE_ROOT_NOT_FOUND=$STATE_ROOT" \
      >> "$STATE_MAP"
